...
Python からブラウザを直接制御し、人間に近い挙動で ChatGPT / Copilot にプロンプトを投げて回答を取得、目視タイミングと DevTools 互換のネットワーク Timing (Stalled / Request sent / Waiting / Content Download) を CSV に記録するクローラ。
...
# 初回のみ: Chrome を起動して OKTA 手動ログイン (default: chatgpt)
python crawler.py --setup

# Copilot 個人向けセッション準備
python crawler.py --setup --target copilot-personal

# Copilot 企業向けセッション準備
python crawler.py --setup --target copilot-corporate
...
# Copilot personal API (wss://copilot.microsoft.com/c/api/chat?)
python crawler.py --target copilot-personal "日本の首都はどこですか？"

# Copilot corporate API (wss://substrate.office/m365Copilot/Chathub/)
python crawler.py --target copilot-corporate "日本の首都はどこですか？"
...
| `target` | `chatgpt` / `copilot-personal` / `copilot-corporate` |
...
|---|---|---|
| `stalled_ms` | `sendStart` までの待ち時間 (Connection Start > Stalled 相当) | BiDi では未提供のため空欄 |
...
| `notes` | セミコロン区切りの補足: `endpoint=...`, `url=...`, `done-by=stop-button` 等, `finish-source=last-dataReceived`, `failed=...`, `diag={...}` |
